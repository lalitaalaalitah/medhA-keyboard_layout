...
| **medhA 6** | Historical | Introduced **AltGr (Right Alt)** layer on Windows for full stop (`.`), comma (`,`), Rupee sign (`₹`), exclamation (`!`), and Vaidika signs without layout switching. |
...
| **macOS 1.5** | Historical | Initial macOS DMG release installer (`medhA-keyboard_mac_1.5_release_1.dmg`) and bundle structure (`medhA-macOSX.1.5.bundle`). |
...
- 🐧 **[Linux Installation Guide](posts/01_Linux_medhA_Keyboard_Installation_Guide.md)**: XKB `sa` symbol file, `evdev.xml` setup, and distro configuration.
- 🪟 **[Windows Installation Guide](posts/01_Windows_medhA_Keyboard_Installation_Guide.md)**: Native MSKLC setup, XP/Vista/7/10/11 activation, & Adishila font setup.
